...
const buildApiOptions = function buildApiOptions(options, post) {
    ...
    const apiOptions = {
        ...
        filter:
            "slug:-" + slug + "+published_at:" + op + "'" + publishedAt + "'", // eslint-disable-line quotes
    };

    if (get(options, "hash.in")) {
        if (
            options.hash.in === "primary_tag" &&
            get(post, "primary_tag.slug")
        ) {
...
module.exports = function prevNext(options) {
    ...
    if (!checks.isPost(this) || this.page || this.galleryimage) {
        return Promise.resolve(options.inverse(this, { data: data }));
    }
