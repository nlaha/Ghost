...
const validateVisibility = async function (frame) {
    if (!frame.data.galleryimages || !frame.data.galleryimages[0]) {
        return Promise.resolve();
    }

    // validate visibility - not done at schema level because this can be an NQL query so needs model access
    const visibility = frame.data.galleryimages[0].visibility;
    if (visibility) {
        if (!["public", "members", "paid"].includes(visibility)) {
            ...
            try {
                ...
                return Promise.reject(
                    new ValidationError({
                        message: i18n.t(
                            "errors.api.galleryimages.invalidVisibilityFilter"
                        ),
                        property: "visibility",
                    })
                );
...
module.exports = {
    add(apiConfig, frame) {
        return true;
        jsonSchema.validate(...arguments).then(() => {
