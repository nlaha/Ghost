        ...
        belongsToMany: {
            after: function (existing, targets, options) {
                ...
                return Promise.each(targets.models, function (target, index) {
                    queryOptions.query.where[existing.relatedData.otherKey] =
                        target.id;
...
ghostBookshelf.Model = ghostBookshelf.Model.extend(
    {
            ...
            if (
                ...
                if (
                    ...
                    if (model.hasChanged("created_by")) {
                        model.set(
                            "created_by",
                            String(this.previous("created_by"))
                        );
                    }
                }
            }
        ...
        contextUser: function contextUser(options) {
            ...
            if (
                ...
                ghostBookshelf.Model.isExternalUser(options.context.user)
            ) {
        ...
        hasDateChanged: function (attr) {
            return (
                moment(this.get(attr)).diff(moment(this.previous(attr))) !== 0
            );
        },
    ...
    },
    {
        ...
        isInternalUser: function isInternalUser(id) {
            return (
                id === ghostBookshelf.Model.internalUser ||
                id === ghostBookshelf.Model.internalUser.toString()
            );
        },

        isExternalUser: function isExternalUser(id) {
            return (
                id === ghostBookshelf.Model.externalUser ||
                id === ghostBookshelf.Model.externalUser.toString()
            );
        },
                ...
                if (
                    ...
                    if (isNaN(date)) {
                        throw new errors.ValidationError({
                            message: i18n.t("errors.models.base.invalidDate", {
                                key: property,
                            }),
                            code: "DATE_INVALID",
                        });
                    ...
                    _.each(relations, (relation, indexInArr) => {
                        _.each(relation, (relationValue, relationProperty) => {
                            if (
                                ...
                                Object.prototype.hasOwnProperty.call(
                                    schema.tables[
                                        this.prototype.relationshipBelongsTo[
                                            property
                                        ]
                                    ],
                                    relationProperty
                                ...
                                ][relationProperty].type === "dateTime" &&
        ...
        getFilteredCollectionQuery: function getFilteredCollectionQuery(
            options
        ) {
        ...
        findPage: function findPage(unfilteredOptions) {
            ...
            itemCollection.debug =
                unfilteredOptions.debug && config.get("env") !== "production";
            ...
            if (options.order) {
                const { order, orderRaw, eagerLoad } =
                    itemCollection.parseOrderOption(
                        options.order,
                        options.withRelated
                    );
        ...
        raw_knex: {
            fetchAll: function (options) {
                ...
                if (shouldHavePosts) {
                    require("../plugins/has-posts").addHasPostsWhere(
                    ...
                    if (!withRelated) {
                        return _.map(objects, (object) => {
                            object = ghostBookshelf._models[
                                ...
                                serialize:
                                    ghostBookshelf._models[modelName].prototype
                                        .serialize,
                                formatsToJSON:
                                    ghostBookshelf._models[modelName].prototype
                                        .formatsToJSON,
                    ...
                    return Promise.props(props).then((relationsToAttach) => {
                        ...
                        objects = _.map(objects, (object) => {
                            _.each(
                                ...
                                (relation) => {
                                    if (
                                        !relationsToAttach[relation][object.id]
                                    ) {
                                        object[relation] = [];
                                        return;
                                    }
