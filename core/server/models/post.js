...
Post = ghostBookshelf.Model.extend(
    {
        ...
        parse() {
            const attrs = ghostBookshelf.Model.prototype.parse.apply(
                this,
                arguments
            );
            ...
            [
                ...
                if (attrs[attr]) {
                    attrs[attr] = urlUtils.transformReadyToAbsolute(
                        attrs[attr]
                    );
                }
            });
        ...
        formatOnWrite(attrs) {
            ...
            if (
                attrs.visibility &&
                !["public", "members", "paid"].includes(attrs.visibility)
            ) {
        ...
        onUpdated: function onUpdated(model, attrs, options) {
            ...
            model.statusChanging =
                model.get("status") !== model.previous("status");
            ...
            model.resourceTypeChanging =
                model.get("type") !== model.previous("type");
            model.publishedAtHasChanged = model.hasDateChanged("published_at");
            model.needsReschedule =
                model.publishedAtHasChanged && model.isScheduled;
            ...
            if (
                model.statusChanging &&
                (model.isPublished || model.wasPublished)
            ) {
        ...
        handleAttachedModels: function handleAttachedModels(model) {
            ...
            model
                ...
                .once(
                    "detaching",
                    function authorsDetaching(collection, author) {
                        model
                            .related("authors")
                            .once(
                                "detached",
                                function authorsDetached(
                                    detachedCollection,
                                    response,
                                    options
                                ) {
            ...
            model
                ...
                .once(
                    "attaching",
                    function authorsAttaching(collection, authors) {
                        model
                            .related("authors")
                            .once(
                                "attached",
                                function authorsAttached(
                                    detachedCollection,
                                    response,
                                    options
                                ) {
        ...
        onSaving: async function onSaving(model, attrs, options) {
            ...
            if (
                ...
                return Promise.reject(
                    new errors.ValidationError({
                        message: i18n.t(
                            "errors.models.post.isAlreadyPublished",
                            { key: "status" }
                        ),
                    })
                );
            ...
            if (newStatus === "scheduled") {
                if (!publishedAt) {
                    return Promise.reject(
                        new errors.ValidationError({
                            message: i18n.t(
                                "errors.models.post.valueCannotBeBlank",
                                { key: "published_at" }
                            ),
                        })
                    );
                    ...
                    return Promise.reject(
                        new errors.ValidationError({
                            message: i18n.t(
                                "errors.models.post.expectedPublishedAtInFuture",
                                {
                                    cannotScheduleAPostBeforeInMinutes:
                                        config.get("times")
                                            .cannotScheduleAPostBeforeInMinutes,
            ...
            if (
                !_.isUndefined(this.get("tags")) &&
                !_.isNull(this.get("tags"))
            ) {
                tagsToSave = [];
            ...
            if (!this.get("mobiledoc")) {
                this.set(
                    "mobiledoc",
                    JSON.stringify(mobiledocLib.blankDocument)
                );
            }
            ...
            if (!options.importing) {
                title =
                    this.get("title") || i18n.t("errors.models.post.untitled");
            ...
            if (
                options.email_recipient_filter &&
        ...
        toJSON: function toJSON(unfilteredOptions) {
            ...
            let attrs = ghostBookshelf.Model.prototype.toJSON.call(
                this,
                options
            );
    ...
    },
    {
        ...
        orderDefaultRaw: function (options) {
            ...
            if (
                options.filter &&
                options.filter.match(/(tags|tag):\s?\[.*\]/)
            ) {
                    ...
                    .then((post) => {
                        return this.findOne(
                            ...
                            if (found) {
                                ...
                                found._previousAttributes =
                                    post._previousAttributes;
        ...
        permissible: async function permissible(
            ...
            function isChanging(attr) {
                return (
                    unsafeAttrs[attr] &&
                    unsafeAttrs[attr] !== postModel.get(attr)
                );
            }
